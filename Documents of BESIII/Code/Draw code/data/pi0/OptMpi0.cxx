...
void OptMpi0()
{
	...
	gStyle->SetOptStat(0);
	TFile *f1 = new TFile("../../0912MCmix/root/pi0MC1.root");
	TFile *f2 = new TFile("../root/0912data1_pi0.root");
	...
	chain1->SetBranchAddress("orig_m2gamma", &chisq1);
	chain2->SetBranchAddress("orig_m2gamma", &chisq2);
	...
	for (int i = 0; i < nentry1; i++)
	{
		...
		for (int j = 0; j < n; j++)
		{
			if (fabs(chisq1 - 0.1349768) < j * h + h)
				s[j]++;
		}
	}
	for (int i = 0; i < nentry2; i++)
	{
		...
		for (int j = 0; j < n; j++)
		{
			if (fabs(chisq2 - 0.1349768) < j * h + h)
				sb[j]++;
		}
	}
	...
	n1->SetXTitle(Form("M(#pi^{0}) (GeV/c^{2})"));
