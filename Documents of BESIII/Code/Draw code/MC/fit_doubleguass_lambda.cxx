...
void fit_doubleguass_lambda()
{
	...
	//------Observvable--------
	RooRealVar M_lamda("M_lamda", "m_{k^{+}k^{-}} (Gev)", 1.111, 1.12);
	...
	RooGaussian gauss("gauss", "title name", M_lamda, sigmean, sigwidth);

	RooRealVar sigmean1("sigmean1", "B^{#pm} mass", 1.1157, 1.111, 1.12);
	RooRealVar sigwidth1("sigwidth1", "B^{#pm} width", 0.006, 0., 0.01);
	RooGaussModel gaussm1("gaussm1", "title name", M_lamda, sigmean1, sigwidth1);
	...
	RooChebychev bkg("bkg", "bkg", M_lamda, RooArgList(c0, c1));
	...
	r_data->Draw("M_lamda>>h1");
	RooDataHist *data = new RooDataHist("data", "data", M_lamda, h1);

	//-------Perform extended ML fit of composite PDF to toy data ------
	sum.fitTo(*data, Extended());

	//-------Plot toy data and composite PDF overlaid--------
	RooPlot *M_lamdaframe = M_lamda.frame();
	//  M_lamdaframe->GetYaxis()->SetTitle("Events GeV/c^{2}");
	M_lamdaframe->GetYaxis()->SetTitle(Form("EVENTS/%.3f (GeV/c^{2})", (up - lo) / Nbin));
	M_lamdaframe->SetXTitle("M_{#Lambda} (GeV/c^{2})");
	M_lamdaframe->SetTitle("");
	M_lamdaframe->GetYaxis()->CenterTitle();
	M_lamdaframe->GetXaxis()->CenterTitle();
	data->plotOn(M_lamdaframe);
	sum.paramOn(M_lamdaframe);
	sum.plotOn(M_lamdaframe);
	sum.plotOn(M_lamdaframe, Components(bkg), LineStyle(kDashed));
	sum.plotOn(M_lamdaframe, Components(gmsum), LineStyle(kDashed));
	M_lamdaframe->Draw();
